...
- **ArcGIS REST (WORKING, free, public):** https://nsgiwa2.novascotia.ca/arcgis/rest/services/PLAN/PLAN_NSPRD_WM84/MapServer — layer 0 `NSPRD.Property`, polygons with a **PID** field, queryable by bbox/point. This is the service the public **Provincial Landscape Viewer** (https://nsgi.novascotia.ca/plv/) uses; host `nsgiwa2` is reachable from the general internet. **This is the default for `ingest-parcels --from-rest`.** (The earlier-documented `gis7.nsgc.gov.ns.ca/ISD_GIS` host is `Internal Services` and times out off the NS-gov network — superseded by this.)
...
- **Bottom line (updated):** the **PLV-backed `nsgiwa2` REST service IS a free, programmatic, public parcel+PID source** — `ingest-parcels --from-rest` works against it. (The *bulk vector download* via DataLocator remains fee/restricted, but it's not needed: query the REST service by bbox.)
